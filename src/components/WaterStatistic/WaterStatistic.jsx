...
import css from '../WaterStatistic/WaterStatistic.module.css';
...
export default function WaterStatistic() {
  const montWater = useSelector(selectMonthWater);
  ...
  const CustomizedDot = props => {
    ...
    return (
      <foreignObject x={cx - 10.5} y={cy - 10.5} width={21} height={21}>
        <span className={css.customDot}></span>
      </foreignObject>
    );
  };
  ...
  const CustomXAxisTick = ({ x, y, payload }) => {
    return (
      <foreignObject className={css.Xtext} x={x} y={y} width={20} height={23}>
  ...
  const CustomTooltip = ({ active, payload }) => {
    if (active && payload && payload.length) {
      return (
        <div className={css.customToolTipWrapper}>
          <div className={css.customToolTip}>
            <p className={css.toolTipText}>{`${payload[0].value} ml`}</p>
          </div>
        </div>
      );
    }
